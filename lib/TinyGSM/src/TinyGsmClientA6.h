...
protected:
  ...
  bool modemConnect(const char* host, uint16_t port, uint8_t* mux, int timeout_s = 150) {  //timeout era 75
    Serial.println();
	Serial.println("entrando a modemconnect");
	unsigned long startMillis = millis();
    uint32_t timeout_ms = ((uint32_t)timeout_s)*1000;
	
	sendAT(GF("+CIPSTART=\"TCP\",190.7.57.163,1883"));
	delay(300);
    if (waitResponse(60000L) != 1) {
		//Serial.println(res);
		return false;
    }
	Serial.println("enviado primer cipstart");
	...
	Serial.println("enviado cipclose");
    sendAT(GF("+CIPSTART="),  GF("\"TCP"), GF("\",\""), host, GF("\",\""), port, GF("\""));
	delay(300);
    //if (waitResponse(timeout_ms, GF(GSM_NL "+CIPNUM:")) != 1) {
	if (waitResponse(95000, GF(GSM_NL "+CIPNUM:")) != 1) {
		Serial.println("gsm_nl: ");
		Serial.println(GSM_NL);
		return false;
    }
	/*
